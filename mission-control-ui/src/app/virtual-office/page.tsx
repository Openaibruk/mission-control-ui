...
const SUPABASE_URL = 'https://vgrdeznxllkdolvrhlnm.supabase.co';
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZncmRlem54bGxrZG9sdnJobG5tIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzM1MTc0MDEsImV4cCI6MjA4OTA5MzQwMX0.LOzByf32QCF0cjYkfiLViXz3Qs04TFp6SkJ040pcDeI';
...
const ROOMS: Room[] = [
  ...
  { id: 'analytics', label: 'Analytics Bay', icon: '📊', agents: ['Amen', 'Yonas', 'Vision'], x: 640, y: 55, w: 270, h: 155, color: '#10b981', glow: 'rgba(16,185,129,0.15)' },
...
const AGENT_DEPT: Record<string, string> = {
  'Nova': 'command', 'Henok': 'dev', 'Kiro': 'dev', 'Cinder': 'dev', 'Cipher': 'dev',
  'Amen': 'analytics', 'Yonas': 'analytics', 'Vision': 'analytics',
  'Nahom': 'marketing', 'Bini': 'marketing', 'Lidya': 'marketing',
  'Onyx': 'security', 'Loki': 'integration',
};
...
export default function VirtualOfficePage() {
  ...
  useEffect(() => {
    const fetchAgents = async () => {
      try {
        const res = await fetch(`${SUPABASE_URL}/rest/v1/agents?select=name,role,status`, {
  ...
  const getAgentStatus = (name: string) => agents.find(a => a.name === name)?.status || 'offline';
      ...
      <div className="max-w-[960px] mx-auto mb-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <a href="http://16.170.169.167:19000" target="_blank" rel="noopener" className="text-[10px] px-3 py-1.5 rounded-lg bg-amber-500/10 text-amber-400 border border-amber-500/20 hover:bg-amber-500/20 transition">
              ⭐ Star Office
            </a>
          </div>
        </div>
