...
// All 17 agents assigned to rooms — no one left without a desk
const ROOMS: Room[] = [
  ...
  return (
    <g style={{ transition: 'opacity 0.5s ease' }}>
      <rect x={x} y={y} width={40} height={20} rx={3}
        ...
        stroke={occupied ? '#4a4a6e' : '#2a2a3e'} strokeWidth={0.5} />
...
export default function VirtualOfficePage() {
  ...
  // Fetch agents ONCE on mount (not every 30s — the main dashboard already handles that)
  useEffect(() => {
    let cancelled = false;
    fetchRef.current++;
    ...
    fetch(`${SUPABASE_URL}/rest/v1/agents?select=name,role,status&limit=50`, {
      ...
      .catch(() => {
        if (!cancelled && myFetch === fetchRef.current) {
          ...
          setAgents(ALL_AGENT_NAMES.map(name => ({
            name,
            role: 'Agent',
            status: 'active',
          })));
  ...
  // Refresh every 2 minutes (not every 30s)
  useEffect(() => {
  ...
  // Clock — update once per minute (we only need to display, not drive status logic)
  useEffect(() => {
    ...
    <div className="min-h-screen bg-[#0a0a0f] p-4 md:p-6">
            ...
            <p className="text-xs text-neutral-500 mt-1">
              {loading ? (
                ...
                <>
                  ...
                  {time.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit', timeZone: 'Africa/Addis_Ababa' })} EAT
                </>
              )}
            </p>
          </div>
      ...
      <div className="max-w-[960px] mx-auto">
        <div className="relative bg-[#0f0f1a] border border-neutral-800/50 rounded-xl overflow-hidden shadow-2xl">
          {/* Loading overlay */}
          {loading && (
            <div className="absolute inset-0 z-50 bg-[#0f0f1a]/80 flex items-center justify-center">
              ...
              return (
                ...
                >
                  ...
                  {room.agents.map((agent, i) => {
                    ...
                    return <Desk key={`desk-${room.id}-${i}`} x={dx} y={dy} occupied={getAgentStatus(agent) !== 'offline'} />;
                  })}
          ...
          {ROOMS.filter(r => r.agents.length > 0).map(room => {
            ...
            return (
              <button key={room.id}
                ...
                <span className="text-neutral-600">{online > 0 && <span className="text-emerald-500/70">({online}/{room.agents.length})</span>}
                </span>
              </button>
            );
          })}
