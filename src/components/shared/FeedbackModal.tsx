...
const CATEGORIES: { id: Category; label: string; icon: React.ReactNode; emoji: string }[] = [
  { id: 'bug', label: 'Bug Report', icon: <AlertTriangle className="w-3.5 h-3.5" />, emoji: '🐛' },
  { id: 'feature', label: 'Feature Request', icon: <Lightbulb className="w-3.5 h-3.5" />, emoji: '💡' },
  { id: 'improvement', label: 'Improvement', icon: <Wrench className="w-3.5 h-3.5" />, emoji: '🔧' },
];

const PRIORITIES: { id: Priority; label: string; color: string; emoji: string }[] = [
  { id: 'low', label: 'Low', color: 'text-blue-400', emoji: '🟦' },
  { id: 'medium', label: 'Medium', color: 'text-amber-400', emoji: '🟨' },
  { id: 'high', label: 'High', color: 'text-orange-400', emoji: '🟧' },
  { id: 'critical', label: 'Critical', color: 'text-red-400', emoji: '🟥' },
];
...
export default function FeedbackModal({ isOpen, onClose, theme }: FeedbackModalProps) {
  ...
  return (
    <ModalWrapper isOpen={isOpen} onClose={onClose}>
      {({ animationPhase }) => (
        <div className={cn(
          "relative modal-panel w-full max-w-lg rounded-xl border shadow-2xl overflow-hidden",
          ...
          <div className="flex items-center justify-between p-4 border-b border-white/5">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 bg-emerald-500/10 rounded-lg flex items-center justify-center">
                {submitted ? <CheckCircle2 className="w-4 h-4 text-emerald-400" /> : <MessageSquarePlus className="w-4 h-4 text-emerald-400" />}
              </div>
              <div>
            ...
            <div className="p-4 space-y-4">
                ...
                <div className="flex gap-2">
                  {CATEGORIES.map(cat => (
                    <button key={cat.id} onClick={() => setCategory(cat.id)}
                      className={cn("flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs border transition-all",
                        category === cat.id
              ...
              <div>
                ...
                <input type="text" value={title} onChange={e => setTitle(e.target.value)}
                  ...
                  className={cn("w-full text-sm px-3 py-2 rounded-lg border outline-none transition-colors",
              ...
              <div>
                ...
                <textarea value={description} onChange={e => setDescription(e.target.value)}
                  ...
                  className={cn("w-full text-sm px-3 py-2 rounded-lg border outline-none resize-none transition-colors",
                    isDark ? "bg-white/5 border-white/10 text-white placeholder:text-neutral-500 focus:border-emerald-500/50" : "bg-gray-50 border-gray-200 text-gray-900 placeholder:text-gray-400 focus:border-emerald-300"
                  )} />
              </div>
